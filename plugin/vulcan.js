...
Vulcan.prototype = {
  ...
  flatten: function flatten(tree, bodyFragment, mainDocUrl, isMainDocument) {
    ...
    if (imports) {
      ...
      var flatHead = dom5.nodeWalkAll(head, function(){ return true; });
      var currentBodyFragment = dom5.constructors.fragment();
      for (var i = 0, im, thisImport; i < imports.length; i++) {
        ...
        var importDoc = this.flatten(im, currentBodyFragment, mainDocUrl);
        // rewrite urls
        ...
        if (importHeadChildren.length) {
          importHeadChildren.forEach(this.reparent(currentBodyFragment));
          currentBodyFragment.childNodes = currentBodyFragment.childNodes.concat(importHeadChildren);
        }
        // defer body children to be inlined in-order
        if (importBodyChildren.length) {
          // adjust body urls to main document
          importBodyChildren.forEach(this.reparent(currentBodyFragment));
          currentBodyFragment.childNodes = currentBodyFragment.childNodes.concat(importBodyChildren);
        }
        ...
        if (!importInBody) {
          ...
          if (container.childNodes.length) {
            dom5.append(currentBodyFragment, container);
          }
        }
        // delete link
        this.removeImportAndNewline(thisImport);
      ...
      // resolve paths
      this.pathResolver.resolvePaths(currentBodyFragment, tree.href, mainDocUrl);
      if (!isMainDocument && currentBodyFragment.childNodes.length) {
        // prepend this import document's <head> into body fragment
        // preserves order when being flattened into a parent import
        var hn = head.childNodes;
        hn.forEach(this.reparent(currentBodyFragment));
        currentBodyFragment.childNodes = hn.concat(currentBodyFragment.childNodes);
        head.childNodes = [];
      }
      dom5.append(bodyFragment, currentBodyFragment);
    }
    return doc;
  },
  ...
  _process: function _process(target, cb) {
    ...
    chain = chain.then(function(){
      ...
      var flatDoc = this.flatten(tree, bodyFragment, tree.href, true);
